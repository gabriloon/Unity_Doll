...
{//RigidBody의 컴포넌트가 있는 들고 싶은 물체에다가 이 Script를 넣는다.
    ...
    public GameObject holdIcon;//Hold아이콘
    ...
    void OnMouseDown()
    {
        ...
        if (Distance < 2.5f)
        {
            ...
            this.transform.parent = GameObject.Find("Destination").transform;//물체를 눈앞으로 끌고 온다.
            postProfile.GetSetting<DepthOfField>().focusDistance.value = changeValue;//자식으로 관계 변경
        }
    }
    void OnMouseUp()
    {
